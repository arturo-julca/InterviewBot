...
public class RecorderManager extends JFrame {
	...
	public static final String JSAVR_OUT_FILE_AUDIO = "./res/InterviewAudio.wav";
	public static final String JSAVR_OUT_FILE_VIDEO = "./res/InterviewVideo.mp4";
	public static final String JSAVR_OUT_FILE_MERGED = "./res/InterviewRecorded.flv";
	...
	public RecorderManager() {
		...
		mainPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), JSAVR_PANEL_TITLE));
	...
	public static void startActionNoUI() throws Exception {
		try {
			checkDirectory();
			...
			System.out.println("Starting the audio recording now...");
	...
	public static boolean stopActionNoUI() {
		...
		merge();
		System.out.println("Audio Video files are merged.");
		return true;
	}
	...
	public static void merge() {
		...
		if (audioFile.exists() && videoFile.exists()) {
			// mav.mergeAudioVideo(JSAVR_OUT_FILE_AUDIO, JSAVR_OUT_FILE_VIDEO,
			// JSAVR_OUT_FILE_MERGED);
			mav.mergeAudioVideo(audioFile.getAbsolutePath(), videoFile.getAbsolutePath(), JSAVR_OUT_FILE_MERGED);
		} else {
			System.out.println("AudioFile " + JSAVR_OUT_FILE_AUDIO + "; not found.");
	...
	private static void checkDirectory() {
		...
		if (!f.exists()) {
			f.mkdir();
			System.out.println(JSAVR_OUT_FILE_DIR + " subdir created...");
		}
	}
